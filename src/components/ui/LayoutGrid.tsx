...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
	...
	return (
		<div className="relative mx-auto grid h-full w-full max-w-[80%] grid-cols-1 gap-4 p-10 md:grid-cols-3">
			{cards.map((card, i) => (
			...
			<motion.div
				...
				className={cn("absolute left-0 top-0 z-10 h-full w-full opacity-0", selected?.id ? "pointer-events-auto" : "pointer-events-none")}
...
const BlurImage = ({ card }: { card: Card }) => {
	...
	return (
		<Image
			...
			className={cn("absolute inset-0 h-full w-full object-cover object-top transition duration-200", loaded ? "blur-none" : "blur-md")}
			alt="thumbnail"
		/>
	);
};
